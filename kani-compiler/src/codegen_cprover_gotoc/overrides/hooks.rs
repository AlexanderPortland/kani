...
impl GotocHooks {
    pub fn hook_applies(&self, tcx: TyCtxt, instance: Instance) -> Option<Rc<dyn GotocHook>> {
        let kani_tool_attr = match KaniFunction::try_from(instance) {
            Ok(KaniFunction::Hook(hook)) => return Some(self.kani_lib_hooks[&hook].clone()),
            Ok(_other) => attributes::fn_marker(instance.def),
            Err(already_calculated) => already_calculated,
        };

        let instance_name = instance.name();
        for h in &self.other_hooks {
            if h.hook_applies(tcx, instance, &instance_name, kani_tool_attr.as_ref()) {
                return Some(h.clone());
            }
        }
        None
